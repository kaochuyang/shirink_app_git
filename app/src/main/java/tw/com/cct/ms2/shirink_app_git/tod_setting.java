...
public class tod_setting extends Base_activity {
    ...
    Button[] gotoSegmenttype = new Button[13];
    Intent plan_setting_xml;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        gotoPlanSettingButtonGroup();//設定轉換頁面按鈕的內容，會用intent.putextra的方式把segmenttype要起始多少帶到下一頁
...
///////////////////////////////////////////////////////////////////////////////////////////////
    ...
    private void gotoPlanSettingButtonGroup() {

        goto_plan_button = findViewById(R.id.goto_plan_button);
        plan_setting_xml = new Intent(tod_setting.this, plan_setting.class);

        Button_goto_where(goto_plan_button, plan_setting_xml);


        for (int i = 8; i < 21; i++) {
            String idname = "segmenttype_button_" + String.format("%d", i);
            int resID = getResources().getIdentifier(idname, "id", getPackageName());
            gotoSegmenttype[i - 8] = findViewById(resID);
            final int finalI = i;


            gotoSegmenttype[i - 8].setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    plan_setting_xml.putExtra("segmenttype", finalI);
                    Log.d("gotoSegmenttype", "onClick: " + plan_setting_xml.getIntExtra("segmenttype", 1));
                    startActivity(plan_setting_xml);
                }
            });
        }
    }
                        ...
                        switch (item.getItemId()) {
                            case R.id.enter:
                                ...
                                for (int segment = 8; segment < 21; segment++) {
                                    if (date_rational_checked(A.getSpecialYear(segment, true), A.getSpecialMonth(segment, true), A.getSpecialDay(segment, true), A.getSpecialYear(segment, false), A.getSpecialMonth(segment, false), A.getSpecialDay(segment, false))) {
                                    } else {
                                        toast_flag = true;
                                ...
                                if (toast_flag) {
                                    Toast toast = Toast.makeText(tod_setting.this, "時段型態日期錯誤 請確認下列時段型態" + errorList.toString(), Toast.LENGTH_LONG);
                                    toast.show();
                                    Log.d("!!!", "onMenuItemClick: " + A.sendSegmentInfoToTc());
                                    Log.d("!!!!", "onMenuItemClick: " + A.sendTODinfoToTc());
                                } else {
                                    tcpClass.send(A.sendSegmentInfoToTc());
...
//                end_day[i] = Integer.valueOf(specialdaycontext[i].getString("end_day"));
            specialday_start_date[Segmenttype - 8].
                    setText(setDateFormat(A.getSpecialYear(Segmenttype, true), A.getSpecialMonth(Segmenttype, true), A.getSpecialDay(Segmenttype, true), Segmenttype, true));
            specialday_end_date[Segmenttype - 8].setText(setDateFormat(A.getSpecialYear(Segmenttype, false), A.getSpecialMonth(Segmenttype, false), A.getSpecialDay(Segmenttype, false), Segmenttype, false));
        }
    }
